...
class QN_S3VM_Dense:
    ...
    def __init__(self, X_l, L_l, X_u, random_generator, **kw):
        ...
        self.__size_l, self.__size_u, self.__size_n = (
            len(X_l),
            len(X_u),
            len(X_l) + len(X_u),
        )
    ...
    def __setParameters(self, **kw):
        ...
        self.__lam_Uvec = [
            float(self.__lamU) * i for i in [0, 0.000001, 0.0001, 0.01, 0.1, 0.5, 1]
        ]
        ...
        self.__regressors_indices = sorted(
            self.__random_generator.sample(range(0, len(self.__X)), self.__numR)
        )
    ...
    def __getFitness_Prime(self, c):
        ...
        term2 = ((-2.0 * self.__s * self.__lamU) / float(self.__size_u)) * (
            preds_unlabeled_squared_exp_f.T * self.__KUR
        ).T
    ...
    def __getTrainingPredictions(self, X, real_valued=False):
        preds = (
            self.__KNR * self.__c[0 : self.__dim - 1, :] + self.__c[self.__dim - 1, :]
        )
        if real_valued == True:
            return preds.flatten().tolist()[0]
        else:
...
class QN_S3VM_Sparse:
    ...
    def __init__(self, X_l, L_l, X_u, random_generator, **kw):
        ...
        if X_l.shape[1] > X_u.shape[1]:
            X_u = sparse.hstack(
                [X_u, sparse.coo_matrix(X_u.shape[0], X_l.shape[1] - X_u.shape[1])]
            )
        elif X_l.shape[1] < X_u.shape[1]:
            X_l = sparse.hstack(
                [X_l, sparse.coo_matrix(X_l.shape[0], X_u.shape[1] - X_u.shape[1])]
            )
    ...
    def __getFitness_Prime(self, c):
        ...
        tmp_unlabeled = (
            self.X_u_T * preds_unlabeled_squared_exp_f
