...
class CerebellumDataset(data.Dataset):
    ...
    def __init__(
        ...
        if mode in ["sup", "unsup", "valid"]:
            ...
            if CerebellumDataset.train_data_sup is None:
                if sup_num is None:
                    assert mode == "unsup"
                    (
                        CerebellumDataset.train_data_unsup,
                        CerebellumDataset.train_labels_unsup,
                    ) = (
                        self.data,
                        self.targets,
                    )
                    ...
                    (
                        CerebellumDataset.train_data_sup,
                        CerebellumDataset.train_labels_sup,
                        CerebellumDataset.train_data_unsup,
                        CerebellumDataset.train_labels_unsup,
                        CerebellumDataset.data_valid,
                        CerebellumDataset.labels_valid,
            ...
            if mode == "sup":
                ...
                self.data = CerebellumDataset.train_data_unsup

                # making sure that the unsupervised labels are not available to inference
                self.targets = (
                    torch.Tensor(CerebellumDataset.train_labels_unsup.shape[0]).view(
                        -1, 1
                    )
                ) * np.nan
            else:
...
class SwapChannels(object):
    ...
    def __call__(self, sample):
        ...
        if self.p > np.random.rand():
            ...
            new_data_point = np.concatenate((new_wvf, acg)).reshape(-1)
...
class VerticalReflection(object):
    ...
    def __call__(self, sample):
        ...
        if self.p > np.random.rand():
            new_wvf = (
                wvf.reshape(N_CHANNELS, CENTRAL_RANGE)[::-1]
                .ravel()
                .copy()
            )
