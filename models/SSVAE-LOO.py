...
def main():
    ...
    for _ in range(10):
        ...
        for idx_in, idx_out in tqdm(loo.split(labelled_indexes), desc="leave-one-out", total=len(labelled_indexes), position=0, leave=True):
            ...
            if ss_vae is None:
                ...
                with torch.no_grad():
                    ...
                    output = classifier(torch.tensor(dataset_out.full_dataset[0], dtype=torch.float32).reshape(1, -1))
        ...
        confusion = confusion_matrix(true_targets, predicted_targets, labels=np.arange(0, 6))
